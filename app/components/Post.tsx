...
function PostContextMenu({ post, user }: { post: any, user: any }) {
    ...
    return (
        <DropdownMenu modal={false}>
            ...
            <DropdownMenuContent className="p-0 flex flex-col">
                <div>
                    <Button containerClass="w-100 flex" className="flex-grow" variant="ghost">
                        <>
                            <Edit className="w-[20px] h-[20px] mr-1" />
                            <span>Edit Post</span>
                        </>
                    </Button>
                </div>
                <fetcher.Form action="/delete-post" method="post">
                    ...
                    <Button containerClass="w-100 flex" className="flex-grow text-red-600 hover:text-red-500"
        ...
        <div className={"flex gap-3 " + className} key={post.id}>
            <div className="flex flex-col gap-2 w-full">
                <div className="flex justify-between items-center gap-3">
                    ...
                    <PostContextMenu post={post} user={user} />
                </div>
